...
def get_system_prompt(
    ...
    return f"""
        ...
        - All math formulas and any text-based schemes (flows, pipelines, trees, arrow-diagrams) MUST be written in LaTeX and wrapped with proper delimiters:
          - Inline: $...$
          - Display: $$...$$
          Do NOT use markdown code blocks or ASCII/Unicode art for schemes; use LaTeX (e.g. $$\\begin{{aligned}} A \\rightarrow B \\rightarrow C \\\\ D \\Rightarrow E \\end{{aligned}}$$).
          If the user provides formulas/schemes in plain text, convert them to LaTeX in your output.
        ...
        - Any mathematical formulas/equations/schemes must be written in LaTeX math mode using `$...$` (inline) or `$$...$$` (block). If the user input contains formulas/schemes not in LaTeX, rewrite them into LaTeX.
